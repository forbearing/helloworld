...
func (m *Machine) expendFields() error {
	if len(m.ProjectId) > 0 {
		p := new(setting.Project)
		if err := database.Database[*setting.Project]().WithCache().Get(p, m.ProjectId); err != nil {
			return err
		}
		m.Project = p
	}
	...
	if len(m.VendorId) > 0 {
		v := new(setting.Vendor)
		if err := database.Database[*setting.Vendor]().WithCache().Get(v, m.VendorId); err != nil {
			return err
		}
		m.Vendor = v
	}
